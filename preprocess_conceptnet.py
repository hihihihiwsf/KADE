...
def add_knowledge2(elem):
    ...
    doc, sen1, sen2, e1_offset, e2_offset, label, s1_w, s2_w, event1_words, event2_words = elem

    event1_knowledge = event1_words + ' ' +  ' '.join(crawl_concept_net(event1_words)[:5]) ## max to 5 piece of knowledge
    event2_knowledge = event2_words + ' ' +  ' '.join(crawl_concept_net(event2_words)[:5])
    # xx = []
    # for i, w in enumerate(event1_knowledge):
    #     tokens = tokenizer.tokenize(w) if w not in ("[CLS]", "[SEP]") else [w]
    #     xx.extend(tokenizer.convert_tokens_to_ids(tokens))
    # xx_len = len(xx)
    
    # yy = []
    # for i, w in enumerate(event1_knowledge):
    #     tokens = tokenizer.tokenize(w) if w not in ("[CLS]", "[SEP]") else [w]
    #     yy.extend(tokenizer.convert_tokens_to_ids(tokens))
    # yy_len = len(yy)
    ...
    temp1 = sen1[:e1_end+1] + xx + sen1[e1_end+1:]
    temp2 = sen2[:e2_end+1] + yy + sen2[e2_end+1:]

    print('_')
    print(elem)
    print([doc, temp2, temp2, list(range(e1_start, e1_end+1)), list(range(e2_start, e2_end+1)), label])

    return [doc, temp1, temp2, e1_offset, e2_offset, label, s1_w, s2_w, event1_words, event2_words]
...
with open('causaltb/causaltb.pickle', 'rb') as f:
    data = pickle.load(f)

data_knowledge = [add_knowledge2(elem) for elem in data]


# causaltb/causaltb_know.pickle:  [doc, temp1, temp2, e1_offset, e2_offset, label, s1_w, s2_w, event1_words, event2_words]
#    temp1 = sen1[:e1_end+1] + [sep] + xx + [sep]+ sen1[e1_end+1:]
#    temp2 = sen2[:e2_end+1] + [sep] +  yy + [sep] +  sen2[e2_end+1:]
#
# causaltb/causaltb_know2.pickle:  [doc, temp1, temp2, e1_offset, e2_offset, label, s1_w, s2_w, event1_words, event2_words]
#    temp1 = sen1[:e1_end+1] + xx +  sen1[e1_end+1:]
#    temp2 = sen2[:e2_end+1] +  yy +  sen2[e2_end+1:]
#

with open('causaltb/causaltb_know2.pickle', 'wb') as f:
